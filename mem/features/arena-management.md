...
**Fase 11.2 — Control Tower (UX-only):** `/arena/dashboard` reorganizado em 5 blocos hierárquicos: (1) Control Tower dominante no topo com OrkymInsightsCard + OrkymActionsCard + caixa de pendências, (2) Operação do dia com KPIs + próximas reservas + atalhos check-in/ocorrências, (3) Financeiro resumido (mês/7d/vencimentos/inadimplência), (4) Torneios, (5) Growth. Sidebar nova `ArenaSidebar` com 5 grupos (Central de Operação / Operação / Financeiro / Torneios / Growth), Pessoas fundido em Operação, Check-in promovido ao topo de Operação, "Ações IA"→"Ações sugeridas", "Autonomia"→"Controle automático". Zero query/lógica/rota alterada — apenas refactor de JSX e relabel.
