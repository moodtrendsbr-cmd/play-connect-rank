...
BEGIN
  ...
  BEGIN
    FOR r IN
      SELECT t.id, t.tenant_id, t.organizer_id, t.name AS tname, t.max_participants AS slots, t.start_date,
  ...
  -- Insight 3 — Inactive athlete (5d+, with prior history)
  BEGIN
    FOR r IN
      SELECT DISTINCT pe.user_id, pe.tenant_id
        FROM public.orkym_proactive_eligibility pe
       WHERE pe.opted_in = true
         AND pe.channel = 'whatsapp'
         AND (
           EXISTS (SELECT 1 FROM public.bookings b WHERE b.user_id = pe.user_id AND b.created_at < now() - interval '5 days')
           OR EXISTS (SELECT 1 FROM public.enrollments e WHERE e.user_id = pe.user_id AND e.created_at < now() - interval '5 days')
         )
         AND NOT EXISTS (
           SELECT 1 FROM public.bookings b WHERE b.user_id = pe.user_id AND b.created_at > now() - interval '5 days'
         )
         AND NOT EXISTS (
           SELECT 1 FROM public.enrollments e WHERE e.user_id = pe.user_id AND e.created_at > now() - interval '5 days'
         )
         AND NOT EXISTS (
  ...
  -- Insight 4 — Classes with open seats
  BEGIN
    FOR r IN
  ...
  -- Insight 7 — Tournament high demand
  BEGIN
    FOR r IN
      SELECT t.id, t.tenant_id, t.organizer_id, t.name AS tname, t.max_participants AS slots, t.start_date,
             (SELECT count(*) FROM public.enrollments e
  ...
  BEGIN
    FOR r IN
      ...
      SELECT a.arena_id, a.tenant_id, a.owner_user_id
        FROM a
       WHERE a.avg3d > 0
