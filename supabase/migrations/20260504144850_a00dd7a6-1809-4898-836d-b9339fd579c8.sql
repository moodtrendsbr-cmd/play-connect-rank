...
DECLARE
  ...
  v_entry_name text;
BEGIN
  -- Only act when status transitions to paid AND we have a modality
  ...
  IF NEW.entry_id IS NOT NULL THEN
    RETURN NEW; -- already linked, idempotent
  END IF;
  ...
  -- Persist back on enrollment without re-firing this trigger via OF status
...
-- Also handle inserts that come in already-paid (rare, but for credit card immediate approval path)
...
DECLARE
  ...
  v_value uuid;
BEGIN
  IF NEW.winner_entry_id IS NULL THEN
    RETURN NEW;
  END IF;
  ...
  -- Fill matches that depend on this one
  FOR v_next IN
...
-- 6. Trigger: auto-fill podium when finals/semifinals close
...
BEGIN
  ...
  -- Semifinal: only auto-fill 3rd if there's no explicit third-place match
  IF NEW.round_number = v_max_round - 1 THEN
    ...
    IF NOT v_third_match_exists AND v_loser_id IS NOT NULL THEN
      -- Insert as 3rd or 4th depending on availability
      INSERT INTO public.modality_placements (modality_id, entry_id, position, tenant_id)
        VALUES (NEW.modality_id, v_loser_id, 3, NEW.tenant_id)
        ON CONFLICT (modality_id, position) DO UPDATE SET entry_id = EXCLUDED.entry_id WHERE FALSE;
      -- If 3 already used, try 4
      IF NOT FOUND THEN
        INSERT INTO public.modality_placements (modality_id, entry_id, position, tenant_id)
